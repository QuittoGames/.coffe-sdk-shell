...
# shell=zsh
#
# env.zsh — Gerenciamento de variáveis de ambiente.
#
...
coffe::env::_default_root() {
    if [[ -f "$ENV_CONF" ]]; then
        ...
        if [[ -n "$line" ]]; then
            print -r -- "${line#ENV_ROOT=}"
            return
        fi
    fi
    print -r -- "$HOME/.secrets"
}
...
coffe::env::_not_found() {
    print -P "${ICON_CLOSE} Not found: $1" >&2
}
...
coffe::env::root() {
    ...
    if [[ -n "$path" ]]; then
        ...
        if [[ -f "$ENV_CONF" ]]; then
            ...
            grep -q "^ENV_ROOT=" "$ENV_CONF" 2>/dev/null || print "ENV_ROOT=$path" >> "$ENV_CONF"
        else
            mkdir -p "${ENV_CONF:h}" 2>/dev/null || true
            print "ENV_ROOT=$path" > "$ENV_CONF"
        fi

        ENV_ROOT="$path"
        print -P "  $(coffe::coffee_icon) ${CLR_GREEN}ENV_ROOT=${CLR_BOLD}$path${CLR_RESET}${CLR_GREEN} (persistido em $ENV_CONF)${CLR_RESET}" >&2
        return 0
    fi
...
coffe::env::list() {
    if [[ ! -d "$ENV_ROOT" ]]; then
        print -P "${ICON_BAN} No secrets directory found" >&2
        return 1
    fi
    ...
    if [[ -z "$files" ]]; then
        print -P "${ICON_BAN} No env files found in $ENV_ROOT${categoria:+/$categoria}" >&2
        return 1
    fi
...
EOF
                ...
                print -P "  ${ICON_FILE} Created: $target"
            else
                print -P "  $(coffe::coffee_icon)  ${CLR_DIM}Already exists:${CLR_RESET} $target"
            fi
        done
    done
...
EOF
        ...
        print -P "  ${ICON_FILE} Created: $ENV_ROOT/global.sh"
    fi
...
coffe::env::load() {
    ...
    if [[ -z "$name" ]]; then
        ...
        if [[ -z "$files" && ! -f "$ENV_ROOT/global.sh" ]]; then
            print -P "${ICON_BAN} No .env files found in $ENV_ROOT" >&2
            return 1
        fi
    ...
    if [[ "$name" == "global" ]]; then
        ...
        source "$ENV_ROOT/global.sh"
        print -P "$(coffe::coffee_icon)  ${CLR_DIM}global loaded${CLR_RESET}"
        return
    fi
...
coffe::env::add() {
    ...
    if [[ ! -d "$env_root" ]]; then
        print -P "  ${ICON_BAN} ${CLR_RED}No secrets directory found${CLR_RESET}" >&2
        return 1
    fi
    ...
    if [[ -z "$files" ]]; then
        print -P "  ${ICON_BAN} ${CLR_RED}No .env files found. Run ${CLR_BOLD}env init${CLR_RESET}${CLR_RED} first.${CLR_RESET}" >&2
        return 1
    fi
    ...
    local selected
    selected=$(coffe::env::_pick_file "$files")
    [[ -z "$selected" ]] && { print -P "  ${ICON_TIMES} ${CLR_CARAMEL}Cancelled${CLR_RESET}" >&2; return; }
    ...
    local key=""
    while [[ -z "$key" ]]; do
        print -n "  ${ICON_TAG} ${CLR_BLUE}Key name${CLR_RESET}: " >&2
        read -r key

        if [[ -z "$key" ]]; then
            print -P "  ${ICON_BAN} ${CLR_RED}Key cannot be empty${CLR_RESET}" >&2
        elif ! coffe::env::_valid_key "$key"; then
            print -P "  ${ICON_BAN} ${CLR_RED}Invalid key name. Use letters, numbers, and underscores only${CLR_RESET}" >&2
            key=""
        elif grep -q "^${key}=" "$full_path" 2>/dev/null; then
            print -P "  ${ICON_WARNING} ${CLR_ORANGE}Key '${key}' already exists in $rel_file${CLR_RESET}" >&2
            print -n "  ${CLR_CARAMEL}Overwrite?${CLR_RESET} (y/N): " >&2
            read -r overwrite
    ...
    local value=""
    print -n "  ${ICON_LOCK} ${CLR_BLUE}Token/value${CLR_RESET} ${CLR_DIM}(hidden input)${CLR_RESET}: " >&2
    read -rs value
    print "" >&2

    if [[ -z "$value" ]]; then
        print -P "  ${ICON_BAN} ${CLR_RED}Value cannot be empty${CLR_RESET}" >&2
        return 1
    fi

    print "" >&2
    print -P "  ${ICON_EYE} ${CLR_BOLD}Preview:${CLR_RESET}" >&2
    print -P "    ${CLR_SKY_BLUE}${key}${CLR_RESET}=${CLR_GREEN}\"${value}\"${CLR_RESET}" >&2

    print "" >&2
    print -n "  ${ICON_QUESTION} ${CLR_CARAMEL}Add this key to ${CLR_BOLD}$rel_file${CLR_RESET}${CLR_CARAMEL}?${CLR_RESET} (y/N): " >&2
    read -r confirm

    if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then
        print -P "  ${ICON_TIMES} ${CLR_CARAMEL}Cancelled${CLR_RESET}" >&2
        return
    fi
    ...
    print -P "  $(coffe::coffee_icon) ${CLR_GREEN}Key ${CLR_BOLD}${key}${CLR_RESET}${CLR_GREEN} added to ${CLR_BOLD}$rel_file${CLR_RESET}" >&2
    print -P "  ${ICON_SHIELD} ${CLR_DIM}File permissions set to 600${CLR_RESET}" >&2
}
...
coffe::env() {
    case "${1:-}" in
        ...
        *)
            ...
            print ""
            print -P "  ${CLR_CARAMEL}root [path]         ${CLR_RESET} ${CLR_DIM}mostra/define a raiz dos envs (persistente)${CLR_RESET}"
            print -P "  ${CLR_CARAMEL}list [cat] [--full] ${CLR_RESET} ${CLR_DIM}lista arquivos (1 por linha, p/ scripts)${CLR_RESET}"
            print -P "  ${CLR_CARAMEL}select [cat]        ${CLR_RESET} ${CLR_DIM}seleciona arquivo (fzf; sem TTY lista)${CLR_RESET}"
            print -P "  ${CLR_CARAMEL}init                ${CLR_RESET} ${CLR_DIM}cria estrutura de categorias + global.sh${CLR_RESET}"
            print -P "  ${CLR_CARAMEL}edit [name]         ${CLR_RESET} ${CLR_DIM}edita global.sh (padrão) ou <name>.env${CLR_RESET}"
            print -P "  ${CLR_CARAMEL}load [name]         ${CLR_RESET} ${CLR_DIM}carrega global.sh + .env, ou um específico${CLR_RESET}"
            print -P "  ${CLR_CARAMEL}add                 ${CLR_RESET} ${CLR_DIM}adiciona chave a um arquivo${CLR_RESET}"
            print -P "  ${CLR_CARAMEL}find <key>          ${CLR_RESET} ${CLR_DIM}busca chave entre os arquivos${CLR_RESET}"
            print -P "  ${CLR_CARAMEL}search <key>        ${CLR_RESET} ${CLR_DIM}alias para find${CLR_RESET}"
            print ""
            ;;
    esac
}
